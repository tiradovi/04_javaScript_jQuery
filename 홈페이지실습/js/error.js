...
function errorFn() {
  ...
  $.get(`https://httpstat.us/${errorVal}`)
    .done(function (data) {
      $("#errorResult").html(`<div class="success">요청성공</div>`);
    })
    .fail(function (xhr, status, error) {
      const 상태코드 = xhr.status || "Unknown";
      const 상태명 = xhr.statusText || error;
      ...
      switch (상태코드) {
        case 404:
          ...
          // 정보를 찾으러 들어간 주소의 위치가 잘못되거나, URL 이상 발생시
          break;
        case 403:
        ...
        case 500:
          ...
          solution = "잠시 후 다시시도하거나 관리자에게 문의하세요";
          //
          break;
        default:
          errorMsg = "알 수 없는 오류가 발생했습니다.";
          solution = "네트워크 상태를 확인하고 다시시도해주세요";
          //
          break;
      }
      $("#errorResult").html(`<div class="error">
